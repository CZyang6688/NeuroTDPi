...
df_x_train = pd.read_csv("NT理化性质train-x.csv", na_values=["?", "NA"])
df_y_train = pd.read_csv("NT理化性质train-y.csv", na_values=["?", "NA"])
df_x_test =pd.read_csv("NT理化性质test-x.csv", na_values=["?", "NA"])
df_y_test = pd.read_csv("NT理化性质test-y.csv", na_values=["?", "NA"])
...
#第一层
model.add(k.layers.Dense(100,input_dim=x_train.shape[1],activation="relu",kernel_regularizer=l1_l2(l1=l11, l2=l22),kernel_initializer='glorot_uniform',use_bias=False))
model.add(BatchNormalization())
model.add(Dropout(dropout))
#第二层
model.add(k.layers.Dense(50,input_dim=100,activation="relu",kernel_regularizer=l1_l2(l1=l11, l2=l22),kernel_initializer='glorot_uniform',use_bias=False))
model.add(BatchNormalization())
model.add(Dropout(dropout))
#第三层：
model.add(k.layers.Dense(25,input_dim=50,activation="relu",kernel_regularizer=l1_l2(l1=l11, l2=l22),kernel_initializer='glorot_uniform',use_bias=False))
model.add(BatchNormalization())
model.add(Dropout(dropout))
#第四层
model.add(k.layers.Dense(1,input_dim=25,activation="sigmoid"))
...
#训练模型
model.fit(x_train,y_train,epochs=1500,batch_size=batch_size,verbose=2, class_weight=class_weights_dict)

# 保存模型
model.save("model_NT理化性质-精简.h5")
